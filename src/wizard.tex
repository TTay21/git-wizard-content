...
\small{\href{https://github.com/benknoble/git-wizard-content}{\texttt{https://github.com/benknoble/git-wizard-content}}}}
...
\begin{frame}{My Journey with Git}
    \begin{itemize}[<+->]
        ...
        \item Blogging! GitHub? \(\Rightarrow\) Commits
        \item Interlude: command-line
        \item Class projects! \(\Rightarrow\) Branches and merges
        \item Hackathon! \(\Rightarrow\) Merge conflicts
...
\begin{frame}[label=setup]{Required setup}
    \begin{enumerate}
        ...
        \item An editor of some kind. There are many available, but VS Code,
            Atom, Sublime, and MacVim are supported out-of-box. Choose one in
            the app's Preferences if you like.
    \end{enumerate}
    Do what?
    \begin{description}
...
\begin{frame}[fragile]{Lumos: Your First \sout{Spell} Commit}
    ...
    \begin{exercise}[Make a commit]
        \begin{enumerate}
            \item Open \texttt{contributors.txt} in your editor (try \texttt{Cmd
                + Shift + A})
...
\begin{frame}{Lumos: Your First \sout{Spell} Commit}
    \begin{figure}
        ...
        \caption{Create \texttt{ben.txt} (1: make changes)}
    \end{figure}
\end{frame}
...
\begin{frame}{Lumos: Your First \sout{Spell} Commit}
    \begin{figure}
        ...
        \caption{Checking out our history}
    \end{figure}

    4: Now we're going to
    \href{https://help.github.com/en/desktop/getting-started-with-github-desktop/creating-your-first-repository-using-github-desktop\#step-6-make-commit-and-push-changes}{push}
\end{frame}
...
\begin{frame}{A Note about Messages}
    \begin{columns}
        ...
        {Here's a good format to follow (click to follow link).}
    \end{columns}
\end{frame}
...
\begin{frame}{Creating a branch}
    \begin{columns}
        ...
        \begin{figure}
            ...
            \caption{1: Using the branch menu}
        \end{figure}

        \column{0.5\textwidth}
        \begin{figure}
            \includegraphics[scale=0.4]{img/create_branch}
            \caption{1: Naming a branch}
        \end{figure}
    \end{columns}
\end{frame}
...
\begin{frame}{Implementing \texttt{add1}}
    \begin{figure}
        ...
        \caption{3: Creating the \texttt{add1} commit}
    \end{figure}
\end{frame}
...
\begin{frame}{Implementing \texttt{sub1} on another branch}
    \begin{figure}
        ...
        \caption{6: Creating the \texttt{sub1} commit}
    \end{figure}
\end{frame}
...
\begin{frame}{Mixing Potions: Your First Merge}
    \begin{exercise}[Merging]
        ...
        {\begin{figure}
            ...
            \caption{1: Setting up the merge}
        \end{figure}}
        \only<2>
        {\begin{figure}
            \includegraphics[scale=0.4]{img/merge_commit}
            \caption{2: Checking the results}
        \end{figure}}
    \end{exercise}
\end{frame}
...
\begin{frame}
    \begin{figure}
        \includegraphics[scale=0.25]{img/new_pr}
        \caption{3: Create a Pull Request UI}
    \end{figure}
\end{frame}
...
\begin{frame}{Results}
    \begin{columns}
        ...
        \begin{figure}
            ...
            \caption{Joe (1/2: adding ``my'' name)}
        \end{figure}
        \begin{figure}
            \includegraphics[scale=0.3]{img/jane}
            \caption{Jane (3/4: adding my partner's name)}
        \end{figure}

        \column{0.5\textwidth}
        \begin{figure}
            \includegraphics[scale=0.35]{img/conflicts_example}
            \caption{4: Attempting to merge. Notice the conflict markers?
            Lines like \texttt{<<<HEAD}, \texttt{===},
            \texttt{>>>branchname}
...
\begin{frame}{Picking sides}
    ...
    conflicts.
    \begin{figure}
        \includegraphics[scale=0.3]{img/merge_results}
        \caption{Choosing Jane's value: the result}
    \end{figure}
\end{frame}
